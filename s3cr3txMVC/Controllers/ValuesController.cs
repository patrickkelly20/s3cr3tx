...
namespace s3cr3txMVC.Controllers
{
    ...
    public class ValuesController : ControllerBase
    {
       ...
       [HttpGet]
        ...
        // GET: api/<ValuesController>
        [HttpPut]
        public string Put(IFormCollection data)//, IFormFile imageFile)
        {
            try {
                //foreach (System.Collections.Generic.KeyValuePair<string,Microsoft.Extensions.Primitives.StringValues> header in Request.Headers)
                //{
                //string strAuth = @"";
                //if (Request.Headers.TryGetValue(@"Authorization", out Microsoft.Extensions.Primitives.StringValues svAuth))
                //{
                //    strAuth = svAuth[0];
                //}
                //string strToken = @"";
                ...
                //    blnD = true;
                //}
                ////Console.WriteLine("Welcome to the Key Creation wizard for PK_s3cr3tx!");
                ////Console.Write(@"Please enter your email address (this will be used as your username): ");
                string strResult = @"";
               string strUserEmail = data["name"];
                ...
                if ((strUserEmail != @"") && IsValidEmail(strUserEmail) && (strMpM != @""))
                {
                    ...
                    //Console.WriteLine(@"Result is: " + didItWork);
                ...
                //    string strGetEmail = //Console.ReadLine();
                //    //Console.Write(@"Please enter your API-Key: ");
                //    string strGetAPIKey = //Console.ReadLine();
                //    //Console.Write(@"Please enter your Authorization Code: ");
                //    string strGetAuthCode = //Console.ReadLine();
                //    Bundle bunCur = Bundle.GetBundle(strGetEmail, strGetAPIKey, strGetAuthCode);
                ...
                //    string strGetDirection = //Console.ReadLine();
                //    string PrivateKey = Encoding.GetEncoding(0).GetString(Encoding.Convert(Encoding.UTF8, Encoding.GetEncoding(0), Convert.FromBase64String(bunCur.PrivateKey)));
                //    string PublicKey = Encoding.GetEncoding(0).GetString(Encoding.Convert(Encoding.UTF8, Encoding.GetEncoding(0), Convert.FromBase64String(bunCur.PublicKey)));

                //    using (RSACryptoServiceProvider r = new RSACryptoServiceProvider())
                //    {

                //        //Enc or Dec input
                //        if (strGetDirection.ToUpper().StartsWith(@"E"))
                //        {
                //            r.FromXmlString(PrivateKey);
                //            string strResult = Convert.ToBase64String(Encoding.Convert(Encoding.GetEncoding(0), Encoding.UTF8, r.Encrypt(Encoding.GetEncoding(0).GetBytes(strInput2), false)));
                //            result = strResult;
                ...
                //        {
                //            r.FromXmlString(PrivateKey);
                //            string strResult = Encoding.GetEncoding(0).GetString(r.Decrypt(Encoding.Convert(Encoding.UTF8, Encoding.GetEncoding(0), Convert.FromBase64String(strInput2)), false));
                //            result = strResult;
                //        }
                //        //Console.WriteLine(@" Your output result is: " + result);
                //         
                //    }
            ...
            catch (Exception ex)
            {
                ...
                SqlParameter p1 = new SqlParameter(@"Source", @"s3cr3txConsole_Main");
        ...
        //{
        //}
        private static string EncResultByID(string strID, string strInput)
        {
            try { 
            string strResult = @"";
            if (strID.Trim().Equals(@"fbfcd562-ba3e-4918-a0fd-942116f2a6c3"))
            {
                string strConnection = @"Data Source=.;Integrated Security=SSPI;Initial Catalog=s3cr3tx";
                SqlConnection sql = new SqlConnection(strConnection);
                SqlCommand command = new SqlCommand();
                command.CommandText = @"dbo.EncDec";
                command.CommandType = System.Data.CommandType.StoredProcedure;
                // SqlParameter p1 = new SqlParameter(@"Auth", strAuth.Trim());
                // SqlParameter p2 = new SqlParameter(@"APIToken", strToken.Trim());
                SqlParameter p3 = new SqlParameter(@"email", @"e8999b8c-117f-43a3-a805-b286ffa65c6f");
                SqlParameter p4 = new SqlParameter(@"input", strInput);
                SqlParameter p5 = new SqlParameter(@"EorD", true);
                SqlParameter p6 = new SqlParameter(@"blnDefault", 1);
                SqlParameter p7 = new SqlParameter(@"strText", System.Data.SqlDbType.NVarChar, -1);
                p7.Direction = System.Data.ParameterDirection.Output;
                //command.Parameters.Add(p1);
                // command.Parameters.Add(p2);
                command.Parameters.Add(p3);
            ...
            {
                string strResult = @"";
        string strSource = @"s3cr3tx.api.ValuesController.EncResultsByIO";
        LogIt(ex.GetBaseException().ToString(), strSource);
                return strResult;
            }
        }

        private static string DecResultByID(string strID, string strInput)
        {
            try { 
            string strResult = @"";
            if (strID.Trim().Equals(@"fbfcd562-ba3e-4918-a0fd-942116f2a6c3"))
            {
                ...
                SqlParameter p3 = new SqlParameter(@"email", @"e8999b8c-117f-43a3-a805-b286ffa65c6f");
                SqlParameter p4 = new SqlParameter(@"input", strInput);
                SqlParameter p5 = new SqlParameter(@"EorD", false);
                SqlParameter p6 = new SqlParameter(@"blnDefault", 1);
                SqlParameter p7 = new SqlParameter(@"strText", System.Data.SqlDbType.NVarChar, -1);
                p7.Direction = System.Data.ParameterDirection.Output;
                //command.Parameters.Add(p1);
            ...
            {
                string strResult = @"";
        string strSource = @"s3cr3tx.api.ValuesController.DecResultsByID";
        LogIt(ex.GetBaseException().ToString(), strSource);
                return strResult;
            }
        }
